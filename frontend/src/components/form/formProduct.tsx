...
export default function FormProduct({closeForm}:any) {
  ...
  const createProduct = async (e: React.FormEvent) =>{
    e.preventDefault()
    if(dataProduct.name == "" || dataProduct.price == 0 || dataProduct.stock == 0){
  ...
  <section className="w-full">
    <h1 className="w-full mb-2 h-12 flex justify-center items-center font-bold text-2xl text-zinc-100 text-center bg-[#000000]">Alta de Productos</h1>
    <article className="flex justify-around items-center mt-10">
      <form className=" border-2 flex flex-col bg-[#65727C] p-6 w-96 rounded-lg" onSubmit={createProduct}>
        <FaArrowLeft className="text-zinc-100  mb-4 text-xl cursor-pointer " onClick={closeForm}/>
        ...
        <label htmlFor="price" className="mt-4  font-bold">Precio</label>
        <div className="flex items-center mt-1">
          <span className="text-gray-400 mr-2">$</span>
          <input onChange={updatePrice} type="number" placeholder="1.00" className="h-8 rounded-md px-2 outline-none w-full"/>
        </div>

        <label htmlFor="providers" className="mt-4  font-bold">Proveedores</label>
        ...
        <label htmlFor="stock" className="mt-4  font-bold">Stock</label>
        <input type="number" onChange={updateStock} placeholder="10" className="h-8 rounded-md px-2 outline-none mt-1"/>

        <label htmlFor="category" className="mt-4  font-bold">Categoría</label>
        ...
        <button className="bg-[#354762] mt-6 h-10 rounded-lg text-zinc-100 font-bold">Agregar</button>
      </form>
